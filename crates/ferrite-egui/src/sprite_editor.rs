...
pub struct SpriteEditorViewport {
    ...
    /// Directory where sprites are saved. On desktop: appdata sprites dir.
    /// On wasm: None (save button is hidden/disabled).
    pub sprites_dir: Option<std::path::PathBuf>,
    ...
    chromakey: ferrite_core::sprite::sheet::ChromakeyConfig,
    picking_chromakey_color: bool,
    /// Platform-agnostic SM storage (list + load TOML). Optional — SM coverage panel
    /// is hidden when None.
    pub sm_storage: Option<Box<dyn crate::sm_storage::SmStorage>>,
}
...
impl SpriteEditorViewport {
    ...
    fn do_export_bundle(&self, sm_source: Option<&str>) {
        let bundle_name = ferrite_core::sprite::editor_state::sanitize_name(&self.state.sprite_name);
        ...
        {
            let _ = (sm_source, png_bytes, json_str);
            log::warn!("bundle export not available on wasm — use download API");
        }
    }
    ...
    fn rebuild_preview_sheet(&mut self) {
        ...
        ferrite_core::sprite::sheet::apply_chromakey(&mut keyed, &self.chromakey);
    ...
    fn do_save(&mut self) {
        ...
        self.state.sm_mappings = self.sm_mappings.clone();
        self.state.chromakey = self.chromakey.clone();
        let sprites_dir = match self.sprites_dir.clone() {
            Some(d) => d,
            None => { log::warn!("save sprite editor: no sprites_dir set"); return; }
        };
        let already_installed = self.state.png_path.parent()
            .map(|p| p == sprites_dir)
            .unwrap_or(false);
        let save_dir = if already_installed {
            self.state.png_path.parent().map(|p| p.to_path_buf())
        } else {
        ...
        if let Some(dir) = save_dir {
            if let Err(e) = self.state.save_to_dir(&dir) {
                log::warn!("save sprite editor: {e}");
            } else {
                let stem = ferrite_core::sprite::editor_state::sanitize_name(&self.state.sprite_name);
                // Update png_path so subsequent saves stay in the sprites dir
                self.state.png_path = dir.join(format!("{stem}.png"));
                self.saved_json_path = Some(dir.join(format!("{stem}.json")));
                self.dirty = false;
            }
        }
    }
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if ui.add_enabled(!s.is_builtin, egui::Button::new("Export PNG\u{2026}")).clicked() {
                        ...
                        #[cfg(target_arch = "wasm32")]
                        {
                            log::warn!("PNG export not available on wasm");
                        }
                    }
        ...
        egui::SidePanel::left("tag_panel").min_width(200.0).show(ctx, |ui| {
            ui.add_enabled_ui(!s.is_builtin, |ui| {
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ui.horizontal(|ui| {
                    ui.heading("Grid");
                    crate::ui_theme::help_icon(ui, "Sets how the PNG is divided into frames. Cols × Rows = total frame count.");
                });
                ui.horizontal(|ui| {
                    ui.label("Cols:");
                ...
                ui.horizontal(|ui| {
                    ui.heading("Tags");
                    crate::ui_theme::help_icon(ui, "Tags group frames into named animations (e.g. 'idle', 'walk'). Select a tag to edit its frame range and direction.");
                });
                ...
                if let Some(tag_idx) = s.selected_tag_idx
                    && tag_idx < s.state.tags.len() {
                        ...
                        crate::ui_theme::hint(ui, "Forward plays frames left-to-right. PingPong bounces back and forth.");
                ...
                {
                    let total = s.total_frames();
                    let color = ferrite_core::sprite::editor_state::SpriteEditorState::assign_color(s.state.tags.len());
                    s.state.tags.push(ferrite_core::sprite::editor_state::EditorTag {
                ...
                // SM selector combobox — always shown; empty list when no storage is set
                {
                    let sm_names: Vec<String> = s.sm_storage.as_ref()
                        .map(|st| st.list_names())
                        .unwrap_or_default();
                    ...
                    // SM coverage panel — shown when an SM is selected and storage is available
                    if let Some(sm_name) = s.selected_sm_name.clone() {
                        // Load and compile the SM source on demand
                        let compiled_sm = s.sm_storage.as_ref()
                            .and_then(|st| st.load(&sm_name))
                            .and_then(|src| {
                                toml::from_str::<ferrite_core::sprite::sm_format::SmFile>(&src)
                                    .ok()
                            })
                            .and_then(|sm_file| {
                                ferrite_core::sprite::sm_compiler::compile(&sm_file).ok()
                            });
            ...
            if let Some(ref tex) = tex {
                ...
                if cols > 0 && rows > 0 && tex_size.x > 0.0 && tex_size.y > 0.0 {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        crate::ui_theme::hint(ui, "Ctrl+scroll to zoom");
                    });
